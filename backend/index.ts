...
const scalingEngines = getScalingEngines(backendService, metricsClient, loggingClient);
const scalingService = new ScalingService(backendService, scalingEngines, loggingClient);
const server = new Server(config, backendService, port, metricsClient, loggingClient, scalingService);
server.init().then(() => {
